...
# critic model，用于预测每一步的动作（生成token）产生的收益
# 使用actor模型进行初始化，并外加一个回归头，输出shape为：(batch_size, seq_len, 1)
...
# 时间序列上的trade off：gamma，控制当前时间和未来时间之间的 return 的 trade off
# advantage 上的trade off：lambd，控制当前时间和未来时间之间的 advantage 的 trade off
...
# 参数名	含义
# kl KL散度，表示当前策略与参考策略（如 SFT 模型）之间的差异
# # shape: (batch_size, seq_len)
# r	外部奖励（如 reward model 对最终 response 打分），shape: (batch_size, 1)
# action_mask	标记 response 部分的位置：1表示有效token，0表示padding
# shape: (batch_size, seq_len)
# kl_ctl	KL控制系数，用于调节 KL 惩罚的权重
# clip_reward_value	对 r 做 clip，避免梯度爆炸
...
def compute_rewards(kl, r, action_mask, kl_ctl, clip_reward_value):
    ...
    # 只给句子最后一个token赋值reward，前面的advantage会通过计算gae传播这个reward。
    for j in range(batch_size):
        rewards[j, :ends[j]][-1] += reward_clip[j, 0]

    return rewards

def generate_experiences(sample_list, actor_model, ref_model, reward_model,
                        critic_model, actor_tokenizer, reward_tokenizer,
                        device):
    ...
    for samples in sample_list:
        ...
        with torch.no_grad():
            ...
            # 计算优势和回报
            advantages, returns=get_advantages_and_returns(value, rewards, action_mask, 
                                                          gamma=0.1, lambd=0.2)
...
def collate_fn(batch):
    ...
    return BufferItem(seqs, action_log_probs, values, returns, advantages,
                      attention_mask, action_mask, action_mask.size(1))

def train_step(experience, steps, actor_model, optimizer_actor, writer,
               critic_model, optimizer_critic):
    ...
    # policy 算 gae
    policy_loss = compute_policy_loss(action_log_probs, old_action_log_probs, advantages, action_mask)
...
def train(episodes, prompts_dataloader, actor_model, max_length, 
          max_new_tokens, n_samples_per_prompt, micro_rollout_batch_size,
          max_epochs):
    # 初始化经验池
    ...
    for episode in range(episodes):
        for rand_prompts in prompts_dataloader:
            # 生成样本（获取模型推理结果）
            samples = generate_samples(rand_prompts, actor_model, max_length, 
                                       max_new_tokens, n_samples_per_prompt,
                                       micro_rollout_batch_size)
            # 生成经验（获取优势，奖励，回报等）
            experiences = generate_experiences(samples)
            buffer.append(experiences)

            dataloader = DataLoader(buffer, batch_size=micro_rollout_batch_size,
...
if __name__ == "__main__":
    ...
    # 从取出的数据中一次遍历多少条（生成经验需要多个模型推理，对显存要求高）
    ...
    # 最大数据token长度
    max_length = 256
    # 实际训练的batch size 大小，一次取多少条数据用于更新参数
    micro_train_batch_size = 2
    # 记录日志
    writer = SummaryWriter('./results/ppo')
    # 策略模型
    ...
    train(episodes, prompts_dataloader, actor_model, max_length,
          max_new_tokens, n_samples_per_prompt, micro_rollout_batch_size,
          max_epochs)